 ...
 *                         S�ren Besenbacher <besen@birc.dk> */
...
SuffixTree_init(SuffixTreeObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (PyBytes_AsStringAndSize(string,&s,&n))   return -1; /* rethrow */
    if (PyBytes_AsStringAndSize(terminal,&t,&n)) return -1; /* rethrow */
...
{
    return PyUnicode_FromStringAndSize(self->tree->str, self->tree->str_len);
}
...
Node_edge_label(NodeObject *self)
{
    ...
    if (start < 0) return PyUnicode_FromStringAndSize("", 0);
    return PyUnicode_FromStringAndSize(self->tree->tree->str + start, length);
}
